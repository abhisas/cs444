...
class YoloLoss(nn.Module):
    ...
    def forward(self, pred_tensor, target_boxes, target_cls, has_object_map):
        ...
        reg_loss = self.get_regression_loss(best_boxes[...,:-1], target_boxes)

        # compute contain_object_loss
        # print(best_boxes[...,4].max(), best_boxes[...,4].min())
        contain_obj_loss = self.get_contain_conf_loss(best_boxes[...,4], best_ious)
        # compute final loss
        total_loss = classification_loss + self.l_noobj*no_obj_loss + self.l_coord*reg_loss + contain_obj_loss
        # construct return loss_dict
        loss_dict = dict(
            total_loss=total_loss / self.batch_size,
            reg_loss=reg_loss / self.batch_size,
            containing_obj_loss=contain_obj_loss / self.batch_size,
            no_obj_loss=no_obj_loss / self.batch_size,
